export default function (product) {
	return `<figure class="product-card relative cursor-pointer" id="${product?.id}" onclick="openProductDetails()">
					<div class="box absolute top-0 right-1.5 z-10">
						<button data-id="${
							product?.id
						}" onclick="editProduct()" class="edit-btn text-sm mr-1 hover:scale-125 transition-transform duration-300">
							<i class="fa-regular fa-pen-to-square"></i>
						</button>
						<button data-id="${
							product?.id
						}" onclick="deleteProduct(event, this)" class="delete-btn text-sm hover:scale-125 transition-transform duration-300">
							<i class="fa-solid fa-xmark"></i>
						</button>
					</div>
					<div class="product-image bg-gray-100 overflow-hidden">
						<img
							src="${product.imgSrc ? product.imgSrc : "../../images_tmp/products/1.webp"}"
							alt=""
